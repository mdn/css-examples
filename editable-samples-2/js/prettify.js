...
(function () {
  ...
  let CPP_KEYWORDS = [COMMON_KEYWORDS,"alignas,alignof,align_union,asm,axiom,bool," +
  ...
  let C_TYPES = /^(DIR|FILE|array|vector|(de|priority_)?queue|(forward_)?list|stack|(const_)?(reverse_)?iterator|(unordered_)?(multi)?(set|map)|bitset|u?(int|float)\d*)\b/;
  ...
  function createSimpleLexer(shortcutStylePatterns, fallthroughStylePatterns) {
    ...
    let decorate = function (job) {
      let sourceCode = job.sourceCode, basePos = job.basePos;
  ...
  function numberLines(node, startLineNum, isPreformatted) {
    ...
    for (let i = 0;  // Number of lines that have been split so far.
  ...
  function recombineTagsAndDecorations(job) {
    ...
    try {
      ...
      while (spanIndex < nSpans) {
        ...
        let end = Math.min(spanEnd, decEnd);
