...
let ie = ie_upto10 || ie_11up
let ie_version = ie && (ie_upto10 ? document.documentMode || 6 : ie_11up[1])
...
let ios = /AppleWebKit/.test(userAgent) && /Mobile\/\w+/.test(userAgent)
...
function lineAtHeight(chunk, h) {
  ...
  outer: do {
    for (let i$1 = 0; i$1 < chunk.children.length; ++i$1) {
      let child = chunk.children[i$1], ch = child.height
...
function collapsedSpanAtSide(line, start) {
  ...
  if (sps) { for (let sp = (void 0), i = 0; i < sps.length; ++i) {
    sp = sps[i]
...
function lineLeft(line) { let order = getOrder(line); return order ? bidiLeft(order[0]) : 0 }
  ...
  return function(str) {
    ...
    for (let i$6 = 0; i$6 < len; ++i$6) {
      if (isNeutral.test(types[i$6])) {
        ...
        let replace$1 = before || after ? "L" : "R"
...
function getMode(options, spec) {
  ...
  if (spec.modeProps) { for (let prop$1 in spec.modeProps)
...
function takeToken(cm, pos, precise, asArray) {
  let getObj = function (copy) { return ({
...
function insertLineContent(line, builder, styles) {
  ...
  for (;;) {
    if (nextChange == pos) { // Update current marker set
      ...
      if (endStyles) { for (let j$1 = 0; j$1 < endStyles.length; j$1 += 2)
        { if (endStyles[j$1 + 1] == nextChange) { spanEndStyle += " " + endStyles[j$1] } } }

      if (!collapsed || collapsed.from == pos) { for (let j$2 = 0; j$2 < foundBookmarks.length; ++j$2)
...
function getUsefulRect(rects, bias) {
  ...
  } } else { for (let i$1 = rects.length - 1; i$1 >= 0; i$1--) {
...
function endOperation_finish(op) {
  ...
  if (unhidden) { for (let i$1 = 0; i$1 < unhidden.length; ++i$1)
...
TextMarker.prototype.clear = function() {
  ...
  if (cm && this.collapsed && !cm.options.lineWrapping) { for (let i$1 = 0; i$1 < this.lines.length; ++i$1) {
  ...
  if (files && files.length && window.FileReader && window.File) {
    ...
    try {
      ...
      if (text$1) {
        ...
        if (selected) { for (let i$1 = 0; i$1 < selected.length; ++i$1)
...
function applyTextInput(cm, inserted, deleted, sel, origin) {
  ...
  for (let i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
    ...
    let changeEvent = {from: from, to: to, text: multiPaste ? multiPaste[i$1 % multiPaste.length] : textLines,
...
function addEditorMethods(CodeMirror) {
  ...
  CodeMirror.prototype = {
    ...
    setSize: methodOp(function(width, height) {
      ...
      let interpret = function (val) { return typeof val == "number" || /^\d+$/.test(String(val)) ? val + "px" : val; }
...
function fromTextArea(textarea, options) {
  ...
  let cm = CodeMirror(function (node) { return textarea.parentNode.insertBefore(node, textarea.nextSibling); },
    options)
  return cm
}
