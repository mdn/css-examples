...
(function(mod) {
  ...
  Completion.prototype = {
    ...
    cursorActivity: function() {
      ...
      let pos = this.cm.getCursor(), line = this.cm.getLine(pos.line);
    ...
    update: function(first) {
      ...
      let self = this, myTick = ++this.tick
  ...
  function isNewCompletion(old, nw) {
    let moved = CodeMirror.cmpPos(nw.from, old.from)
  ...
  function Widget(completion, data) {
    ...
    this.data = data;
    this.picked = false;
    let widget = this, cm = completion.cm;
    ...
    if (overlapY > 0) {
      let height = box.bottom - box.top, curTop = pos.top - (pos.bottom - box.top);
  ...
  function resolveAutoHints(cm, pos) {
    let helpers = cm.getHelpers(pos, "hint"), words
    if (helpers.length) {
  ...
  CodeMirror.registerHelper("hint", "fromList", function(cm, options) {
    ...
    if (token.string && /\w/.test(token.string[token.string.length - 1])) {
      let term = token.string, from = CodeMirror.Pos(cur.line, token.start);
    } else {
      let term = "", from = to;
    }
    let found = [];
